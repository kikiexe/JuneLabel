                ...
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 relative">
                    ...
                    `}>
                        ...
                        <div className="mb-8 block lg:hidden">
                            <label className="text-xs font-bold uppercase tracking-wider mb-2 block opacity-70">Search</label>
                        ...
                        {/* Categories */}
                        <div className="mb-8">
                            <h3 className="font-serif text-lg font-bold border-b border-[#7C634D]/20 pb-2 mb-4">Categories</h3>
                            <ul className="space-y-3 text-sm">
                                <li>
                                    <button 
                                        ...
                                        className={`hover:underline underline-offset-4 transition-all ${!filters.category ? 'font-bold pl-2 border-l-2 border-[#7C634D]' : 'opacity-70'}`}
                                    >
                                        All Products
                                    </button>
                                </li>
                                {categories.map(cat => (
                                    <li key={cat.id}>
                                        <button 
                                            ...
                                            className={`text-left hover:underline underline-offset-4 transition-all ${filters.category === cat.slug ? 'font-bold pl-2 border-l-2 border-[#7C634D]' : 'opacity-70'}`}
                                        >
                                            {cat.name}
                                        </button>
                                    </li>
                    ...
                    <div className="w-full lg:w-3/4">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
                            ...
                            <div className="relative w-full sm:w-64">
                                ...
                                />
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#7C634D]/50" size={16} />
                            </div>

                            <div className="flex items-center gap-4 w-full sm:w-auto">
                                <span className="text-xs uppercase tracking-wider opacity-60 hidden sm:inline">Sort By:</span>
                            ...
                            <div className="mb-6 flex gap-2 flex-wrap text-xs">
                                {filters.search && (
                                    <span className="bg-[#7C634D] text-[#FFF6EC] px-3 py-1 rounded-full flex items-center gap-2">
                                        ...
                                        <button onClick={() => { setSearch(''); handleFilterChange('search', ''); }}><X size={12} /></button>
                                    </span>
                                )}
                                {filters.category && (
                                    <span className="bg-[#7C634D] text-[#FFF6EC] px-3 py-1 rounded-full flex items-center gap-2">
                                        Category: {categories.find(c => c.slug === filters.category)?.name || filters.category}
                                        <button onClick={() => { setSelectedCategory(''); handleFilterChange('category', ''); }}><X size={12} /></button>
                                    </span>
                                )}
                                <button 
                        ...
                        {products.data.length > 0 ? (
                            <>
                                ...
                                {products.data.map((product) => (
                                    <Link key={product.id} href={route('product.detail', product.slug)} className="group block">
                                        <div className="relative overflow-hidden mb-4 bg-[#f9f9f9] aspect-[3/4]">
                                            ...
                                            {!product.is_active && (
                                                <div className="absolute inset-0 bg-white/50 flex items-center justify-center">
                                                    <span className="bg-gray-800 text-white px-3 py-1 text-xs uppercase font-bold">Out of Stock</span>
                                                </div>
                                            )}
                                        </div>
                                ...
                                <div className="mt-16 flex justify-center gap-2">
                                    {products.links.map((link, i) => (
                                        link.url ? (
                                            <Link
                                                ...
                                                className={`px-4 py-2 text-sm font-medium border transition-colors ${
                                                    link.active 
                                                        ...
                                                        : 'bg-white text-[#7C634D] border-[#7C634D]/20 hover:bg-[#7C634D]/5'
                                                }`}
                                            ...
                                            <span 
                                                key={i}
                                                className="px-4 py-2 text-sm text-gray-400 border border-gray-100 cursor-not-allowed"
                                                dangerouslySetInnerHTML={{ __html: link.label }}
                                            />
                                        )
                                    ))}
