...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (product, quantity = 1) => {
    ...
    if (maxStock === 0) {
      showNotification('⚠️ Produk ini sedang habis!');
      return;
    }
    ...
    setCartItems((prevItems) => {
      ...
      if (existingItemIndex > -1) {
        ...
        if (newQty > maxStock) {
          showNotification(`⚠️ Stock tidak cukup! Hanya ${maxStock} tersedia.`);
        ...
        return [
          ...prevItems,
          {
            ...
            stock: maxStock, // Store stock for cart validation
            quantity,
          },
        ];
      }
    ...
    setCartItems((prevItems) =>
      prevItems.map((item) => {
        if (item.id === productId) {
          const maxStock = item.stock || 999; // Fallback to high number if stock not set
